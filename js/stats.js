...
(function(h, n) {
	...
	function q(c, a, b) {
		var d = window.location.host,
			...
			f = d.split(".");
		2 < f.length && (d = (e[f.slice(-2)
				.join(".")] ? f.slice(-3) : f.slice(-2))
			.join("."));
	...
	function y() {
		try {
			...
			if ((m = c.plugins) && (k = m.length))
				...
				} else n = (new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))
					.GetVariable("$version"), b.fl = n.replace(/^.*\s+(\d+)\,(\d+).*$/, "$1.$2")
		} catch (p) {
			return {}
		}
		return b
	...
	function A() {
		...
		for (a in k) {
			var b = p(k[a].c_id),
				d;
			"afs" == a ? d = (d = /ssid=([^&]*)/i.exec(l()
				.hash)) && d[1] ? d[1] : "" : (d = l()
				.param, d = d[k[a].id] ? d[k[a].id] : "");
		...
		var a = window.location,
			...
			e = function() {
				t(("https:" == document.location.protocol ? "https://" : "http://") + "tajs.qq.com/ping_hotclick_min.js", function() {
					window.hotclick && (new hotclick(c))
						.watchClick()
				})
			};
		1 * h.pf ? (new RegExp(b))
			.test(h.hot.url) && e() : (a = h.sid, t("http://tcss.qq.com/heatmap/" + a % 100 + "/" + v(a) + ".js?random=" + +new Date, function() {
				var a;
